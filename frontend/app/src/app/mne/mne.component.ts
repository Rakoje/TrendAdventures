...
@Component({
  selector: 'app-mne',
  templateUrl: './mne.component.html',
  styleUrls: ['./mne.component.css']
})
export class MneComponent implements OnInit {
  ...
  book() {
    ...
    if (this.error) {
      this.message += "Molimo Vas da popunite sva obavezna polja.\nProvjerite da li je mejl u odgovarajućem formatu."
    } else {
      ...
      this.bookingService.reserve(this.reservation).subscribe((resp) => {
        if (resp['message'] == 'reserved') {
          this.enabled = false;
          this.reserved = "Čestitamo! Vaša avantura je rezervisana. Uskoro ćete dobiti potvrdu na Vaš mejl! Provjerite spam folder."
  ...
  dateChange() {
    ...
    if (dateText[3] === '1') {
      this.enabled = false;
      this.message = "Datuma koji ste izabrali nije predviđen prolazak. Molimo Vas odaberite drugi termin.";
    }
  }
  ...
  numberOfGuests(num) {
    ...
    if (num !== undefined) {
      for (var i = 0; i < num; i++) {
        var guest = this.htmlToElement("<div class=\"col-sm-12\">Gost " + (i + 1) + "</div>");
